...
impl ResonantCollinearity {
    ...
    fn points_on_line(&self, pair: Vec<&[isize; 2]>) -> Vec<[isize; 2]> {
        ...
        points.push(*pair[0]);    
        points.extend(self.extend_line(pair[0], pair[1]));

        points.push(*pair[1]);
        points.extend(self.extend_line(pair[1], pair[0]));
    ...
    fn extend_line(&self, start: &[isize; 2], end: &[isize; 2]) -> Vec<[isize; 2]> {
        ...
        let mut m = 1;
        loop {
            let np = Self::point_at_line(start, end, m);
            if !Self::fits_bounds(self, &np) {
                break;
            }
            result.push(np);
            m += 1;
