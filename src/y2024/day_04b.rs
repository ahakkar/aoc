            ...
            if let Some(&idx) = map.get_idx(x, y) {
                ...
                for dir in [
                    ...
                    for i in 0..4 {
                        ...
                        if loc.x < 0
                            || loc.x >= width as isize
                            || loc.y < 0
                            || loc.y >= height as isize
                        {
                            pat1_match = false;
                            pat2_match = false;
                            break;
...
pub fn gold(data: &[String]) -> usize {
    ...
    for y in 1..(data.len() - 1) {
        for x in 1..(data[0].len() - 1) {
            ...
            if (nw == Some('M') && se == Some('S') || nw == Some('S') && se == Some('M'))
                && (ne == Some('M') && sw == Some('S')
                    || ne == Some('S') && sw == Some('M'))
            {
                sum += 1;
...
mod tests {
    ...
    #[test]
    fn test_test() {
        let test_data: Vec<String> = read_data_from_file("input/2024/test/04.txt");
    ...
    #[test]
    fn test_silver() {
        let test_data: Vec<String> = read_data_from_file("input/2024/real/04.txt");
