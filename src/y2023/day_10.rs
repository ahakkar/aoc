...
lazy_static! {
    static ref NODE_CHARS: HashSet<char> = ['|', '-', 'L', 'J', '7', 'F', 'S']
        .iter()
        .cloned()
        .collect();
}
...
impl Fro for PipeMaze {
    fn fro(input: &str) -> Self {
        ...
        Self {
            ...
            map_width: graph_data.map_width,
            map_height: graph_data.map_height,
        }
    }
}
...
impl Solution for PipeMaze {
    ...
    */
                ...
                if NODE_CHARS.contains(char) {
                    ...
                    if let Some(node_index) = ctni.get(&coord) {
                        for neighbour_coord in
                            get_neighbours(data, &coord, char, map_width, map_height)
                        {
        ...
        if let Some(start_coord) = start {
            if let Some(start_index) = ctni.get(&start_coord) {
                if let Some(loop_path) = find_loop(&graph, *start_index) {
                    return TaskResult::Usize((loop_path..len() + 1) / 2);
                }
            }
        }
        TaskResult::String("failed".to_string())
    }
    ...
    fn gold(&self) -> TaskResult {
        ...
        TaskResult::Usize(count) // 3033 "not the right answer" .. 337 new guess..
...
impl PipeMaze {
    ...
    fn get_enhanced_char_representation(
        e: char,
        p: char,
    ) -> HashMap<char, Vec<Vec<char>>> {
        let mut representations = HashMap::new();
        representations.insert('L', vec![vec![e, p, e], vec![e, p, p], vec![e, e, e]]);

        representations.insert('J', vec![vec![e, p, e], vec![p, p, e], vec![e, e, e]]);

        representations.insert('F', vec![vec![e, e, e], vec![e, p, p], vec![e, p, e]]);

        representations.insert('7', vec![vec![e, e, e], vec![p, p, e], vec![e, p, e]]);

        representations.insert('|', vec![vec![e, p, e], vec![e, p, e], vec![e, p, e]]);

        representations.insert('-', vec![vec![e, e, e], vec![p, p, p], vec![e, e, e]]);

        representations
    }

    fn write_path_to_2d_map_file(
        graph_data: &PipeGraph,
        file_path: &str,
    ) -> io::Result<()> {
        let mut file = File::create(file_path)?;
        let mut map = vec![
            vec![' '; (graph_data.map_width as usize) * 3];
            (graph_data.map_height as usize) * 3
        ];
                ...
                if let Some(chars) = &graph_data.chars {
                    ...
                    for node in path {
                        ...
                        if let Some(representation) =
                            representations.get(&chars[coord.y][coord.x])
                        {
...
mod tests {
    ...
    #[test]
    fn test() {
        let test_data = read_data_from_file("input/2023/test/10.txt");
        let queue = PipeMaze::fro(&test_data);
    ...
    #[test]
    fn real() {
        let real_data = read_data_from_file("input/2023/real/10.txt");
        let queue = PipeMaze::fro(&real_data);

        assert_eq!(queue.silver(), TaskResult::Usize(6820));
        assert_eq!(queue.gold(), TaskResult::Usize(337));
    }
}
